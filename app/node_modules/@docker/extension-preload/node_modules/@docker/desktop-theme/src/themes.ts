...
import type { DockerPalette } from './docker-color-palettes';
import {
...
/**
 * THEMING TECH DEBT
 * * ad-hok links styling in client/desktop-ui/src/extensions/marketplace/details/HTMLContentWrapper.tsx
 */

interface PaletteAugmentations {
  type: ThemeName;
  status: {
    running: string;
    starting: string;
  ...
  instruction: {
    ...
    successContainerBackground: string;
    successContainerShadow: string;
    successTooltipBackground: string;
  ...
  images: {
    ...
    headerDeletionBackgroundColor: string;
  ...
  docker: DockerPalette;
}

declare module '@mui/material/styles' {
  interface BreakpointOverrides {
    xs: false; // removes the `xs` breakpoint
    sm: true; // adds `sm` breakpoint
    md: true;
    lg: true;
    xl: false;
  }
  ...
  interface Palette extends PaletteAugmentations {}

  // eslint-disable-next-line no-shadow, @typescript-eslint/no-empty-interface
  interface PaletteOptions extends Partial<PaletteAugmentations> {}

  // TODO: remove once the new design system is in place
  interface TypographyVariants {
    /* eslint-disable camelcase */
    h1_ds: CSSProperties;
    h2_ds: CSSProperties;
    h3_ds: CSSProperties;
    subtitle_ds: CSSProperties;
    body1_ds: CSSProperties;
    body2_ds: CSSProperties;
    caption_ds: CSSProperties;
    button_ds: CSSProperties;
    /* eslint-enable camelcase */

    'inline-code': CSSProperties;
  }
...
declare module '@mui/material/styles/createMixins' {
  ...
  interface Mixins {
    ...
    customScrollBar: {
      ...
      whenHovered: Record<string, any>;
    };
    tables: {
      // We created this when we first introduced datagrids
      // to the app for the volumes/containers/etc tables, because
      // we thought these styles might not apply to all datagrids.
      // Potentially they'll just become universal datagrid styles in
      // the future - let's decide when we add the next datagrid.
      customDataGrid: Record<string, any>;
    };
    iconButtons: {
      primaryColorOnHover: Record<string, any>;
    };
  }

  // eslint-disable-next-line no-shadow
  interface MixinsOptions {
    checkboxDescription?: Mixins['checkboxDescription'];
    customScrollBar?: Mixins['customScrollBar'];
    tables?: Mixins['tables'];
    iconButtons?: Mixins['iconButtons'];
  }
}

function createCommonPaletteOptions(pal: DockerPalette) {
  const text = {
    primary: pal.text,
 ...
 * to extensions.
 */
function getCustomThemeOptions(getTheme: () => Theme): ThemeOptions {
 ...
 */
function getStandardThemeOptions(
  themeName: ThemeName,
  palette: DockerPalette,
): ThemeLayer {
    ...
    return {
      ...
      components: {
        MuiDataGrid: {
          styleOverrides: {
            ...
            row: {
              ...
              '&:hover': {
                backgroundColor:
                  themeName === 'dark' ? '#303E46' : colors.white5, // Should '#303E46' be a palette color?
              },
            },
            cell: {
              borderColor:
                themeName === 'dark' ? palette.grey[500] : palette.grey[300],
            },
            columnHeaders: {
              textTransform: 'uppercase',
              borderColor:
        ...
        MuiCard: {
          styleOverrides: {
            root: {
              borderRadius: '4px !important', // Todo: shoud this value set in the theme instead (theme.shape.borderRadius)?
              borderColor:
                themeName === 'dark' ? palette.grey[400] : palette.grey[100],
              backgroundColor:
                themeName === 'dark' ? palette.grey[200] : '#ffffff',
        ...
        MuiFilledInput: {
          styleOverrides: {
            root: {
              ...
              backgroundColor:
                themeName === 'dark' ? alpha('#ffffff', 0.09) : '#ECECEC',
              '&:focus, &:hover': {
                backgroundColor:
                  themeName === 'dark' ? alpha('#ffffff', 0.13) : '#E4E4E4',
              },
            },
            underline: {
              '&:after': {
 ...
 * MUI theme object.
 */
function getStandardDarkPaletteOptions(palette: DockerPalette): ThemeLayer {
  return () => {
 ...
 */
function getCustomDarkPaletteOptions(getTheme: () => Theme): ThemeOptions {
  const theme = getTheme();
...
function getDockerPaletteOptions(pal: DockerPalette): ThemeLayer {
  return () => ({
    palette: {
      docker: pal,
    },
 ...
 * MUI theme object.
 */
function getStandardLightPaletteOptions(palette: DockerPalette): ThemeLayer {
  return () => {
    const common = createCommonPaletteOptions(palette);
 ...
 * This layer specifies custom palette properties
 */
function getCustomLightPaletteOptions(getTheme: () => Theme): ThemeOptions {
  const theme = getTheme();
...
function getExtensionsOptions(mainAppThemeOptions: ThemeOptions): ThemeLayer {
  function assumeIsObjectStyleOptions(
    typographyOptions: ThemeOptions['typography'],
  ): TypographyOptions | undefined {
  ...
  const typographyOptions = assumeIsObjectStyleOptions(
    mainAppThemeOptions.typography,
  );
...
export const appThemes: {
  readonly [theme in ThemeName]: ThemeOptions;
} = {
