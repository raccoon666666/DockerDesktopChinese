...
export type ThemeLayer = (getTheme: () => Theme) => ThemeOptions;
 ...
 */
export function composeThemeOptionsFromLayers(
  layers: ThemeLayer[],
): ThemeOptions {
  return layers.reduce<ThemeOptions>((previousOptions, currentLayer) => {
    const currentOptions = currentLayer(
      memoize(() => createTheme(previousOptions)),
    );
